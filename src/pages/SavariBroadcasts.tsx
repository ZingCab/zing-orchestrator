...
export default function SavariBroadcastsPage() {
  ...
  const subtitle = rawCount > 0 ? `${parsedAll.length} for your fleet · ${rawCount} in feed` : "Etios & Wagon R · rule-based";
    ...
    <SavariShell active="feed" title="Live Feed" subtitle={subtitle}>
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {!q.isLoading && !q.isError && rawItems.length === 0 && (
          <div className="mc-card" style={{ textAlign: "center", padding: "40px 24px" }}>
            <RefreshCw size={28} style={{ color: "var(--text-body-secondary)", margin: "0 auto 12px" }} />
            <p style={{ font: "700 15px var(--font-heading)", color: "var(--text-heading)" }}>No live broadcasts right now</p>
            <p style={{ font: "400 12px/1.6 var(--font-body)", color: "var(--text-body-secondary)", marginTop: 6, maxWidth: 380, marginInline: "auto" }}>
              Savaari's feed is empty at the moment. If this persists, check the balance &amp; KYC status above — a
              negative balance or pending KYC pauses new business.
            </p>
          </div>
        )}
